...
export const ExperienceSection = () => {
  ...
  return (
    <SectionContainer as="section" className="relative pb-14 sm:pb-16 lg:pb-18">
      <div
        aria-hidden="true"
      ...
      <div className="relative z-10 flex flex-col gap-8 lg:gap-10">
        <AnimatedHeading
          ...
          subtitle="Experiências profissionais que consolidaram minha base técnica, comunicação com usuários e capacidade de resolver problemas com responsabilidade."
          size="lg"
          accent
        />
        <div className="flex flex-col gap-5">
          ...
          <p className="max-w-2xl text-sm leading-6 text-muted">
            Cada etapa consolidou minha base técnica e reforçou minha atuação em ambientes reais, com organização, iniciativa e comunicação direta com usuários e equipes.
          </p>
        </div>
        <div className="grid gap-5 lg:grid-cols-2">
          {experiences.map((experience, index) => (
            ...
            >
              <GlassCard className="flex h-full flex-col gap-5 p-5 sm:p-6 hover:border-accent/50 transition-colors">
                ...
                <div className="space-y-3">
                  <h3 className="text-lg font-semibold text-fg sm:text-xl">{experience.role}</h3>
                  <p className="text-sm leading-6 text-muted">{experience.summary}</p>
                  {experience.impact ? (
                    <span className="text-[0.65rem] uppercase tracking-[0.22em] text-accent font-semibold">
                      {experience.impact}
                    </span>
                  ) : null}
                ...
                <div className="flex flex-wrap gap-2 pt-0.5">
                  {experience.tags.map((tag) => (
                    <span
                      key={tag}
                      className={cn(
                        'rounded-full border border-white/10 bg-surface/60 px-2.5 py-1 text-[0.6rem] uppercase tracking-[0.18em] text-muted hover:border-accent/30 transition-colors',
                      )}
                    >
                      {tag}
                    </span>
